...
Definitions`NumericQ = {Hold[ NumericQ[e_] := If[NumberQ[N[e]], True, False]; ]};
...
Definitions`Equal = {Hold[ Equal[a_?NumericQ, b_?NumericQ] := (a//N) == (b//N); ]};
...
Definitions`Positive = {Hold[ Positive[x_?NumberQ] := x > 0; ]};
...
Definitions`Negative = {Hold[ Negative[x_?NumberQ] := x < 0; ]};
...
Definitions`PossibleZeroQ = {Hold[ PossibleZeroQ[e_] := e === 0 || e === 0.; ]};
Tests`PossibleZeroQ = {
    ESimpleExamples[
...
Definitions`MinMax = {Hold[ MinMax[l_List] := {Min[l], Max[l]}; ]};
Tests`MinMax = {
    ESimpleExamples[
...
Definitions`Element = {Hold[ Element[i_Integer, Integers] := True; ]};
Tests`Element = {
    ESimpleExamples[
