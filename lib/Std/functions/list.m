...
Definitions`Total = {Hold[Total[l__List] := Apply[Plus, l];]}
...
Definitions`Mean = {Hold[Mean[l__List] := Total[l] / Length[l];]}
...
(*incorrect results.*)
Definitions`Table = {Hold[Table[a_, b_Integer] := Table[a, {UniqueDefined`i, 1, b}];]}
...
Definitions`ParallelTable = {Hold[ParallelTable[a_, b_Integer] := ParallelTable[a, {UniqueDefined`i, 1, b}];]};
...
Definitions`AppendTo = {Hold[AppendTo[list_, e_] := (list = Append[list, e]);]}
...
Definitions`PrependTo = {Hold[PrependTo[list_, e_] := (list = Prepend[list, e]);]}
...
Definitions`Last = {Hold[ Last[e_?((Length[#]>=1)&)] := e[[Length[e]]]; ]};
...
Definitions`First = {Hold[ First[e_?((Length[#]>=1)&)] := e[[1]]; ]};
...
Definitions`Rest = {Hold[ Rest[e_?((Length[#]>=1)&)] := e[[2;;All]]; ]};
...
Definitions`ListQ = {Hold[ ListQ[expr_] := Head[expr] === List; ]};
...
Definitions`Count = {Hold[ Count[l_, pattern_] := Count[l, pattern, {1}]; ]};
...
Definitions`Tally = {Hold[ Tally[l_List] := {#, Count[l, #]} & /@ DeleteDuplicates[l]; ]};
...
Definitions`Attributes = {Hold[ Attributes[ConstantArray] = {Protected};
Definitions`ConstantArray = {Hold[ ConstantArray[c_, n_Integer] := Table[c, n]; ]}; ]};
Tests`ConstantArray = {
    ESimpleExamples[
