...
Definitions`Sec = {Hold[ Sec[x_ - Pi/2] := Csc[x]; ]};
Attributes[Sec] = {Listable, NumericFunction, Protected};

Definitions`Csc = {Hold[Csc[inner : Verbatim[Plus][Repeated[_ * I]]] := -I * Csch[-I * inner // Distribute];]};
Attributes[Csc] = {Listable, NumericFunction, Protected};

Definitions`Cosh = {Hold[ Cosh[a_]*Csch[a_]^(b_Integer?Positive)*rest___ := Coth[a]*Csch[a]^(b - 1)*rest ]};
Attributes[Cosh] = {Listable, NumericFunction, Protected};
Definitions`ArcSin = {Hold[
...
Definitions`TrigReduce = {Hold[ TrigReduce[a_] := (Print["Unsupported call to TrigReduce", a];a); ]};
Attributes[TrigReduce] = {Protected};

Definitions`TrigToExp = {Hold[
...
Definitions`RotationMatrix = {Hold[
