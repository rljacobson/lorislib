...
Definitions`FoldList = {Hold[ FoldList[f_, expr_] := FoldList[f, First[expr], Rest[expr]] ]};
...
Definitions`Nest = {Hold[ Nest[f_, expr_, n_] := Last[NestList[f, expr, n]] ]};
...
Definitions`NestWhile = {Hold[ NestWhile[args__] := Last[NestWhileList[args]] ]};
Tests`NestWhile = {
    ESimpleExamples[
...
Definitions`FixedPointList = {Hold[ FixedPointList[f_, expr_] := NestWhileList[f, expr, UnsameQ, 2] ]};
...
Definitions`Identity = {Hold[ Identity[expr_] := expr; ]};
