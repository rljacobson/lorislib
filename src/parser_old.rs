...
use crate::{
  atoms::{Function, StringLiteral, Symbol, Variable, Sequence, SequenceVariable},
...
fn literal(input: &str) -> IResult<&str, RcExpression> {
  ...
  let expr = Rc::new(StringLiteral(val.1.to_string()).into());
...
mod tests {
  ...
  fn test_items() {
    ...
    let _result: (&str, MatchEquation) = match_expression(match_expr).unwrap();

    // If all of the unwraps above succeed then this test has succeeded.
    assert_eq!(2+2, 4);
  }
}
