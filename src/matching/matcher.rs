...
pub trait Matcher{
  ...
  // Every matcher must also have a static method `try rule` that checks if it
  ...
  // implements `Matcher`, not a `Matcher` trait object, I don't see a way
  ...
  fn next(&mut self) -> Option<SmallVec<MatchStack>>;
}
